...
$default_vehicle_maximum_number = 20;
...
$default_communication_packet_ttl_for_delivery_ratio = 1800; #TTL for delivery ratio comparison; unit is seconds.
...
#@@For Upload Mode: For Multiple APs
@SEEDS = (1, 100, 1, 1, 0); #seed
...
print("Put the comparison target [1: EDD-Link model, 2: EDD model, 3: TBD's EDD computation type, 4: Edge delay model, 5: Intersection Forwarding type, 6: Average Convoy Length estimation type for one road segment, 7: Average Link Delay estimation for one road segment, 8: Target Point Selection type, 9: Target Point Computation Method] (default: 1): ");
...
elsif($comparison_target < 1 || $comparison_target > 9)
{
...
print("\nPut the evaluation type [1: vehicle maximum number, 2: vehicle packet generating entity number, 3: vehicle speed, 4: vehicle speed standard deviation, 5: vehicle interarrival time, 6: communication packet interarrival time, 7: communication packet TTL, 8: communication packet delivery probability threshold, 9: AP maximum number, 10: simulation time, 11: AP passing vehicle percentage] (default: 1): ");
...
elsif($evaluation_type < 1 || $evaluation_type > 11)
{
...
#number of measures for performance evaluation, such as expected delivery delay, actual delivery delay, the ratio of two delivery delays, packet delivery ratio, expected delivery delay standard deviation, and delivery delay difference between the expected delivery delay and actual delivery delay; if we add other measures, we need to increase $num_of_measures by the number of the additional measures
$num_of_measures = 6;
#$num_of_measures = 6;
